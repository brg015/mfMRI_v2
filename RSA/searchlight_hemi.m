function [out] = searchlight_hemi(subject,cursub)
global SL;
%=========================================================================%
...
% Special Output
if SL.ERmatch==1
    % Now setup special output maps
    phase={'R' 'E' 'ER'};
    for ii=3 %:3
        M=length(out);
        out{M+1}.mat=nan(prod(SL.V.dim),1);  
        out{M+1}.name=[phase{ii} '_bwSubject_IDon_mean'];
        out{M+2}.mat=nan(prod(SL.V.dim),1);  
        out{M+2}.name=[phase{ii} '_bwSubject_IDoff_mean'];
        out{M+3}.mat=nan(prod(SL.V.dim),1);  
        out{M+3}.name=[phase{ii} '_bwSubject_ID_pairT'];
        out{M+4}.mat=nan(prod(SL.V.dim),96);  
        out{M+4}.name=[phase{ii} '_bwSubject_ID_4D_Trial_zT'];
        out{M+5}.mat=nan(prod(SL.V.dim),1);  
        out{M+5}.name=[phase{ii} '_N'];
        out{M+6}.mat=nan(prod(SL.V.dim),1);  
        out{M+6}.name=[phase{ii} '_NaN'];
        % Ignore individual differences for now.
%         out{M+5}.mat=nan(prod(SL.V.dim),1);  
%         out{M+5}.name=[phase{ii} '_idfSubject_IDon_mean'];
%         out{M+6}.mat=nan(prod(SL.V.dim),1);  
%         out{M+6}.name=[phase{ii} '_idfSubject_IDoff_mean'];
%         out{M+7}.mat=nan(prod(SL.V.dim),1);  
%         out{M+7}.name=[phase{ii} '_idfSubject_ID_pairT'];
    end
end
% for ii=1:length(out)
