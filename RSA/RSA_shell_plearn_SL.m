...
for cursub = 1:length(SL.dir.subjects)
    ...
    SL.design.cBox=cumsum(SL.design.Box); 
    for ii=1:NModels
        SL.design.matrix{ii}=[];   % Initialize model
        SL.design.power{ii}=[];
        % No need for design matrices if MVPA
        if ~(strcmp(SL.design.calc{ii},'MVPA') || strcmp(SL.design.calc{ii},'distance'))
            RSA_design_matrix_v3(ii); 
        else
            % Figure out the class labels for classification-like things
            SL.design.classID{ii}=[ones(SL.design.Box(SL.design.class{ii}(1)),1);...
                zeros(SL.design.Box(SL.design.class{ii}(2)),1)];
            if SL.design.class{ii}(1)==1, I1=1:SL.design.cBox(1); 
            else I1=SL.design.cBox(SL.design.class{ii}(1)-1)+1:SL.design.cBox(SL.design.class{ii}(1)); end 
            if SL.design.class{ii}(2)==1, I2=1:SL.design.cBox(2); 
            else I2=SL.design.cBox(SL.design.class{ii}(2)-1)+1:SL.design.cBox(SL.design.class{ii}(2)); end 
            SL.design.trial_include{ii}=[I1 I2]'; clear I1 I2;
        end
    end
    if flag==1 
